...
export class CommunicationComponent implements OnInit {
  ...
  setupDataStream() {
    if (this.member === true) {
      this.service.getCommunicationInboxMessages().subscribe(a => {
        this.inboxData = a;
        this.inboxDataSource.data = this.inboxData;
      });
    }
    if (this.management === true) {
      this.service.getCommunicationOutboxMessages().subscribe(a => {
        this.outboxData = a;
        this.outboxDataSource.data = this.outboxData;
      });
    }
    if (this.chair === true) {
      this.service.getPendingCommunicationMessages().subscribe(a => {
        this.pendingData = a;
  ...
  aproveDocument(id) {
    this.service.aprovePendingCommunicationMessage(id).subscribe(a => {
      this.setupDataStream();
  ...
  declineDocument(id) {
    this.service.declinePendingCommunicationMessage(id).subscribe(a => {
      this.setupDataStream();
      this.openSnackBar("Document status has been updated", "close");
    });
  }
